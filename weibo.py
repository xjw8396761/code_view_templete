...
class Weibo(object):
    ...
    def user_to_mysql(self):
        ...
        self.mysql_insert(mysql_config, 'weibo_user', [self.user])
    ...
    def get_pics(self, weibo_info):
        ...
        if weibo_info.get('pics'):
            ...
            pic_list = [ pic['large']['url'].replace("large","mw690")  for pic in pic_info]
            pics = ','.join(pic_list)
        else:
            pics = ''
        return pics
    ...
    def standardize_date(self, created_at):
        ...
        if u"刚刚" in created_at:
            created_at2 = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            created_at = datetime.now().strftime("%Y-%m-%d")
        elif u"分钟" in created_at:
            minute = created_at[:created_at.find(u"分钟")]
            minute = timedelta(minutes=int(minute))
            created_at2 = (datetime.now() - minute).strftime("%Y-%m-%d %H:%M:%S")
            created_at = (datetime.now() - minute).strftime("%Y-%m-%d")
        elif u"小时" in created_at:
            hour = created_at[:created_at.find(u"小时")]
            hour = timedelta(hours=int(hour))
            created_at2 = (datetime.now() - hour).strftime("%Y-%m-%d %H:%M:%S")
            created_at = (datetime.now() - hour).strftime("%Y-%m-%d")
        elif u"昨天" in created_at:
            day = timedelta(days=1)
            created_at2 = (datetime.now() - day).strftime("%Y-%m-%d %H:%M:%S")
            created_at = (datetime.now() - day).strftime("%Y-%m-%d")
        elif created_at.count('-') == 1:
            #print (created_at)
            #sys.exit(0)
            year = datetime.now().strftime("%Y")
            created_at2 = year + "-" + created_at
            created_at = year + "-" + created_at
        elif created_at.count('-') == 2:
            created_at2 = created_at
        else:
            # Sat May 23 03:36:17 +0800 2020
            created_at2 = str(datetime.strptime(created_at, '%a %b %d %H:%M:%S +0800 %Y'))
            created_at = created_at2[:10]
        return (created_at,created_at2)
        ...
        try:
            ...
            if retweeted_status:  # 转发
                ...
                if is_long_retweet:
                    ...
                    retweet = self.parse_weibo(retweeted_status)
                retweetinfo = self.standardize_date(
                    retweeted_status['created_at'])
                retweet['created_at'] = retweetinfo[0]
                retweet['created_at2'] = retweetinfo[1]
            ...
            #print(retweetinfo)
            #sys.exit (0)
            weibo['created_at'] ,weibo['created_at2']  = retweetinfo
    ...
    def get_filepath(self, type):
        ...
        try:
            ...
            )[0] + os.sep + 'weibo' + os.sep + self.user['id']
            #screen_name
    ...
    def weibo_to_mysql(self, wrote_count):
        ...
        # 在'weibo'表中插入或更新微博数据
        self.mysql_insert(mysql_config, 'weibo_info', retweet_list)
        self.mysql_insert(mysql_config, 'weibo_info', weibo_list)
...
def main():
    try:
        ...
        print (since_date ,aid)
        user_id_list = [sys.argv[1]]
        #user_id_list.append(aid)
        print (user_id_list)
...
if __name__ == '__main__':
    try:
        aid, since_data,filter = sys.argv[1],sys.argv[2],sys.argv[3]
	#print (aid,quality)
    except Exception:
        print ("请输入正确的参数1")
        sys.exit(0)
